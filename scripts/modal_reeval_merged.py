...
CKPT_DIR = RUN / "full_qwen3vl" / "ckpt_final"
MERGED_DIR = CKPT_DIR / "merged_eval"
EVAL_TAG = "merged_qwen3vl"

# shas asserted before any use (RSVQA-ADAPT manifest + local verification)
CKPT_SHA256 = {
    ...
    "adapter_config.json":
        "1df4b0afd120aecae7147cd47af194d40d6f9ca9538c0877b22e71ea25a579a1",
    "mix.jsonl":
...
app = modal.App("satquery-reeval-merged")
...
def stage_check() -> dict:
    ...
    _write_json(RUN / "reeval_merged" / "stage_check.json", report)
    vol.commit()
    return _js(report)
...
def smoke_merged(n: int = 200) -> dict:
    ...
    _write_json(RUN / "reeval_merged" / "smoke_merged.json", report)
    vol.commit()
    return _js(report)
